...
# PCPs — physicians who log in and upload images
PCPS = [
    {
...
# Patient users — patients who can log in (role=PATIENT)
PATIENT_USERS = [
    {"email": "alice.johnson@email.com", "full_name": "Alice Johnson"},
    {"email": "bob.williams@email.com", "full_name": "Bob Williams"},
    {"email": "carol.davis@email.com", "full_name": "Carol Davis"},
    {"email": "eva.thompson@email.com", "full_name": "Eva Thompson"},
]
...
def build_sql() -> str:
    ...
    # --- Users: PCPs ---
    lines.append("-- ============================================================")
    lines.append("-- PCP users")
    lines.append("-- ============================================================")
    for pcp in PCPS:
        lines.append(dedent(f"""\
            INSERT INTO users (email, password_hash, full_name, role, npi_number)
            VALUES ('{pcp["email"]}', '{pw_hash}', '{pcp["full_name"]}', 'PCP', '{pcp["npi_number"]}')
    ...
    for pu in PATIENT_USERS:
        lines.append(dedent(f"""\
            ...
            VALUES ('{pu["email"]}', '{pw_hash}', '{pu["full_name"]}', 'PATIENT')
            ON CONFLICT (email) DO NOTHING;"""))
    lines.append("")
...
def print_credentials():
    ...
    print("=" * 70)
    print()
    print(f"{'Email':<35} {'Name':<22} {'Role':<10} {'Patients'}")
    print(f"{'-'*35} {'-'*22} {'-'*10} {'-'*8}")
    print(f"{'sarah.chen@hospital.org':<35} {'Dr. Sarah Chen':<22} {'PCP':<10} 4")
    print(f"{'james.patel@hospital.org':<35} {'Dr. James Patel':<22} {'PCP':<10} 2")
    print(f"{'maria.garcia@hospital.org':<35} {'Dr. Maria Garcia':<22} {'PCP':<10} 1")
    print(f"{'robert.kim@hospital.org':<35} {'Dr. Robert Kim':<22} {'PCP':<10} 0")
    print(f"{'alice.johnson@email.com':<35} {'Alice Johnson':<22} {'PATIENT':<10} —")
    print(f"{'bob.williams@email.com':<35} {'Bob Williams':<22} {'PATIENT':<10} —")
    print(f"{'carol.davis@email.com':<35} {'Carol Davis':<22} {'PATIENT':<10} —")
    print(f"{'eva.thompson@email.com':<35} {'Eva Thompson':<22} {'PATIENT':<10} —")
    print()
